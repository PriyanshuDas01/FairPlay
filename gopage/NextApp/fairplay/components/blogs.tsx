  ...
  return (
    <>
      <main className="min-h-screen bg-slate-900">
        ...
        <header className="border-b border-gray-700 py-2">
          <div className="container mx-auto px-4 py-6">
            <div className="space-y-5">
              <div className="inline-block rounded-lg bg-green-700 px-3 py-1 text-sm text-green-100">
                Sports & Wellness Journal
              </div>
        ...
        {/* Articles */}
        <section className="container mx-auto px-4 py-12">
          <article className="relative overflow-hidden rounded-xl bg-green-800 text-green-100">
            <div className="relative z-10 grid gap-4 p-8 md:grid-cols-2 md:p-12 lg:p-16">
              <div className="space-y-4">
                <div className="text-sm font-medium">Tell Us Your Story</div>
                <h2 className="text-3xl font-bold tracking-tight sm:text-4xl">
                  The Road to Recovery: From Professional Athletes to Mental Health Advocates
                </h2>
                <p className="text-green-300">
                Share your journey in the sports and antidoping field. Let the world get inspired by you!
                </p>
          ...
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {blogs.slice(0, 3).map((blog) => (
              <article key={blog._id} className="group cursor-pointer space-y-4" onClick={() => handleBlogClick(blog)}>
                ...
                <div className="space-y-2">
                  <h3 className="text-xl font-semibold text-green-300">{blog.title}</h3>
                  <p className="text-green-400">{blog.content.substring(0, 100)}...</p>
                </div>
              </article>
            ))}
          </div>
            ...
            <div ref={scrollContainerRef} className="flex overflow-x-auto space-x-6 pb-4 scrollbar-hide">
              {blogs.slice(3).map((blog) => (
                <article key={blog._id} className="flex-shrink-0 w-64 cursor-pointer space-y-4" onClick={() => handleBlogClick(blog)}>
                  ...
                  <div className="space-y-2">
                    <h3 className="text-lg font-semibold text-green-300">{blog.title}</h3>
                    <p className="text-green-400">{blog.content.substring(0, 50)}...</p>
                  </div>
                </article>
              ))}
            </div>
            <button onClick={scrollRight} className="absolute right-0 top-1/2 transform -translate-y-1/2 bg-green-700 p-2 rounded-full shadow-md z-10">
              <ChevronRight className="w-6 h-6 text-green-200" />
            </button>
          </div>
        </section>
      </main>
      ...
      {selectedBlog && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
          <div className="bg-green-900 p-8 rounded-lg max-w-2xl w-full max-h-[80vh] overflow-y-auto relative">
            <button onClick={handleClosePopup} className="absolute top-4 right-4">
              <X className="w-6 h-6 text-green-300" />
            </button>
            <h2 className="text-2xl font-bold mb-4 text-green-300">{selectedBlog.title}</h2>
            {selectedBlog.thumbnail && (
              <Image
                src={selectedBlog.thumbnail}
                alt={selectedBlog.title}
            ...
            <p className="text-green-400 mb-4">{selectedBlog.content}</p>
            <p className="text-sm text-green-500">By {selectedBlog.author} on {new Date(selectedBlog.createdAt).toLocaleDateString()}</p>
          </div>
        </div>
      )}
    </>
